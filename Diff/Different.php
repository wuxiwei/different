...
class Different
{
     ...
     * @param $before
     * @param $end
     * @param null $keys    索引数组值相同时，两个key不一定相同，所以用两个$keys和$ks分别存储key值
     * @param null $ks
     */
...
//                        $this->_assignment($end, $k, $e[$k], null, $keys);
                        ...
                        $this->_assignment($end, $k, $e, null, $keys);
        ...
        foreach ($array as $k => $v) {
            if (is_array($v)) {
                ...
                if(array_key_exists($key,$re)){
                    //如果存在key值，表示返回的数组多包含一层，需要根据$key取值。
                    $this->_assignment($reArr, $k, $re[$key], null, $keys);
                }else{
                    $this->_assignment($reArr, $k, $re, null, $keys);
                }
